...
SETTINGS_FILE = os.path.join(
    os.path.expanduser("~"), ".adm_video_downloader_settings.json"
)
...
class VideoDownloaderApp(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.setWindowTitle("ADM Video Downloader v2.2.1")
        ...
        self.video_title_entry.setPlaceholderText(
            "Video title will appear here automatically"
        )
    ...
    def setup_queue_handlers(self):
        ...
        self.queue_handler.download_complete_signal.connect(
    ...
    def save_app_settings(self):
        ...
        try:
            ...
            UIHelpers.show_warning(
                self, "Settings Error", f"Could not save settings: {str(e)}"
            )
    ...
    def handle_enable_fetch(self, data):
        ...
        self.format_selector.set_fetching_state(
            False
        )  # Assuming enable_fetch means fetching is done
    ...
    def browse_folder(self):
        ...
        folder = UIHelpers.browse_folder(
            self, "Select Download Folder", self.folder_entry.text()
        )
        if folder:
    ...
    def open_download_folder(self):
        ...
        if not folder:
            UIHelpers.show_warning(
                self, "Error", "Please select a download folder first"
            )
            return
    ...
    def start_download(self):
        ...
        # Start the download
        success, error_msg = self.downloader.start_download(
            url, type_choice, format_str, folder, user_title
        )
        if not success:
    ...
    def select_default_format(self):
        ...
        try:
            ...
            UIHelpers.show_warning(
                self, "Error", f"Could not open format selection: {str(e)}"
            )
    ...
    def toggle_theme(self):
        ...
        try:
            ...
            UIHelpers.show_warning(
                self, "Settings Error", f"Could not change theme: {str(e)}"
            )
    ...
    def toggle_auto_fetch(self, checked):
        ...
        try:
            ...
            UIHelpers.show_warning(
                self, "Settings Error", f"Could not change auto-fetch setting: {str(e)}"
            )
    ...
    def toggle_remember_directory(self, checked):
        ...
        try:
            ...
            UIHelpers.show_warning(
                self, "Settings Error", f"Could not change directory setting: {str(e)}"
            )
